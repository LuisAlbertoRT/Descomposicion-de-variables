...
plot(Data$Date, Data$Close, type = "l",
     main="Precio de cierre ETF Ultra S&P 500", #mean es para poner titulos a las graficas
...
ultra <- ggplot(Data, aes(x=Data$Date, y=Data$Close)) +
  ggtitle("Precio de ETF Ultra S&P 500, Cierre : Enero 2015-Noviembre 2020") +
...
ultra2 <- Data %>%
  ...
  ggtitle("Precio de Cierre ETF Ultra S&P 500: Enero 2015-Noviembre 2020") +
...
ultra_des <- ts(Data$Close, start = c(2015,11,04), frequency = 252) #ts convierte en series tiempo la serie
...
autoplot(pronostico) + geom_forecast(h=10) + #h es el n?mero de d?as a pronosticar
  ylab("Precio de cierre") +
  xlab("Tiempo") +
...
subcojunto <- window(ultra_des, start = 2018)
autoplot(subcojunto) + geom_forecast(h=150)+
  ggtitle("Pronostico de S&P 500 a 150 dias, w= 252")

#Guarda los resultados

write.csv(pronostico, "C:/Users/USER/Dropbox/CLASES/ECONOMETRIA FINANCIERA/SEMESTRE 2019 AGOSTO DICIEMBRE/PLANTILLAS/Pronostico 252 PE?OLES.csv")
...
Resultados_des <- "C:/Users/USER/Dropbox/CLASES/ECONOMETRIA FINANCIERA/SEMESTRE 2019 AGOSTO DICIEMBRE/PLANTILLAS/RESULTADOS DESCOMPOSICI?N/"
contador1 = 1

for (contador1 in 195:200){
  Des_n<-ts(Data$Close, start = c(2014,08,19), frequency = contador1)
  print(Des_n)
